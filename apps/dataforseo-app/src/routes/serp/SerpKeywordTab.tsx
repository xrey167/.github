...
import { type SerpLiveBatch, type SerpResultItem } from "../../lib/tauri";
...
interface Props {
  ...
  apiCall: (args: {
    ...
    languageCode: string;
    depth: number;
  }) => Promise<SerpLiveBatch>;
}

export default function SerpKeywordTab({ hint, apiCall }: Props) {
    ...
    <div className="flex flex-col gap-6">
      ...
      {batch && (
        <div className="flex items-center justify-between text-xs text-slate-500">
          <span>
            {batch.items.length} items for &ldquo;{batch.keyword}&rdquo; · actual {formatUsd(batch.cost_usd)} ·
